...
class SkirtSim:
    ...
    def GasFile(self):
        ...
        # Debugging: Print dust temperature range after temperature filtering
        print(f"\n--- Dust Temperatures after temp_mask ({len(temp_filt)} particles) ---")
        if len(temp_filt) > 0:
            print(f"Min Temp: {np.min(temp_filt):.2f} K, Max Temp: {np.max(temp_filt):.2f} K, Mean Temp: {np.mean(temp_filt):.2f} K")
        else:
            print("No dust particles remaining after temperature filter.")
        ...
        # Ensure 'center' is an array with 3 elements (x,y,z)
        if not isinstance(center, np.ndarray) or center.shape != (3,):
            print(f"Warning: 'Center (pc)' from snapshot header is not a 3-element array: {center}")
            # Attempt to convert if it's a list or tuple, or default if it's a scalar
            if isinstance(center, (list, tuple)) and len(center) == 3:
                center = np.array(center)
            elif np.isscalar(center): # If it's a scalar, assume [scalar, scalar, scalar]
                center = np.array([center, center, center])
            else: # Fallback to 0,0,0 if all else fails
                center = np.array([0., 0., 0.])
                print("Defaulting center to [0,0,0] due to unexpected format.")
        ...
        # Debugging: Print dust temperature range after radial cut
        print(f"--- Dust Temperatures after radial cut ({len(final_temp)} particles) ---")
        if len(final_temp) > 0:
            print(f"Min Temp: {np.min(final_temp):.2f} K, Max Temp: {np.max(final_temp):.2f} K, Mean Temp: {np.mean(final_temp):.2f} K")
        else:
            print("No dust particles remaining after radial cut.")
